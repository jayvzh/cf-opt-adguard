...
- **全自动**：自动从 AGH 日志发现高频站点，确认 Cloudflare 后写入优选 IP，新站点自动加入、失效站点自动淘汰。
- **判得准**：DNS + IP 段 + 响应头多重信号确认 Cloudflare；拿不准的域名逐条精确重写，不误伤其他站点。
- **优选 IP 接入 CloudflareSpeedTest**：直接读取 CFST 的 `result.csv`，v4/v6 区分，多 IP 择优。
- **安全可控**：全程本地运行、不上传查询日志；默认只预览计划，确认后才写入。

## 它如何工作

```text
AGH 查询日志 → 找出高频域名 → 确认是 Cloudflare → 优选 IP
            → 生成计划（新增/更新/删除）→ 写入 AGH 重写规则 → 回查验证
```
...
```bash
sudo bash <(curl -sL go2.gg/cf-opt) \
...
构建、测试、项目结构与设计文档入口见 [DEVELOPMENT.md](DEVELOPMENT.md)。
